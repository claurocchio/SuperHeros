...
public class GrupoMongoDaoTest {

	private GrupoMongoDao dao;
	private FiltroGrupo.Builder filterBuilder;

	public GrupoMongoDaoTest() {
		dao = new GrupoMongoDao();
	...
	public void cloningTest() {
		...
		Grupo miGrupo = dao.findOne(filtros);
		/*
		 * try{ miGrupo = dao.findOne(filtros); }catch(ManyResultsException e){
		 * Collection<Grupo> resultados = e.getResults(Grupo.class);
		 * assertEquals(2, resultados.size());
		 * 
		 * for(Grupo unGrupo : resultados){ assertEquals("iddddsssss", new
		 * Long(1), unGrupo.getId()); } }
		 */
	...
	public void updateTest() {
		...
		assertEquals(true, miGrupo.getPersonajes() == null || miGrupo.getPersonajes().isEmpty());
		assertEquals("pepito1", miGrupo.getNombre());
